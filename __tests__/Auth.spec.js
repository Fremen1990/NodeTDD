...
describe('Token Expiration', () => {
  ...
  it('returns 403 when token is older than 1 week', async () => {
    ...
    const validaUpdate = { username: 'user1-updated' };
    const response = await putUser(savedUser.id, validaUpdate, { token: token });
    expect(response.status).toBe(403);
  });
  ...
  it('refreshes lastUsedAt when unexpired token is used', async () => {
    ...
    await putUser(savedUser.id, validaUpdate, { token: token });
