...
describe('Post Hoax', () => {
  ...
  it.each`
    language | message
    ${'pl'}  | ${pl.unauthorized_hoax_submit}
  ...
  it.skip('associates hoax with attachment in database', async () => {
    const uploadResponse = await uploadFile();
    const uploadedFileId = uploadResponse.body.id;
    await addUser();
    await postHoax({ content: 'Hoax content', fileAttachment: uploadedFileId }, { auth: credentials });
    const hoaxes = await Hoax.findAll();
    const hoax = hoaxes[0];
  ...
  it('keeps the old associated hoax when new hoax submitted with old attachmentId', async () => {
    const uploadResponse = await uploadFile();
    const uploadedFileId = uploadResponse.body.id;
    await addUser();
    await postHoax({ content: 'Hoax content', fileAttachment: uploadedFileId }, { auth: credentials });
    const attachment = await FileAttachment.findOne({ where: { id: uploadedFileId } });
    await postHoax({ content: 'Hoax content 2', fileAttachment: uploadedFileId }, { auth: credentials });
